...
{
    // 只允许以下后缀名的图片文件上传
    protected $allowedExt = ['png', 'jpg', 'git', 'jpeg'];
    ...
    public function save($file, $folder, $filePrefix, $maxWidth = false)
    {
        ...
        // 文件具体存储的物理路径， `public_path` 获取 `public` 文件夹的物理路径。
        // 值如：/home/vagrant/Code/larabbs/public/uploads/images/avatars/201709/21/
        $uploadPath = public_path() . '/' . $folderName;

        // 获取文件的后缀名，因图片从剪贴板里黏贴时后缀名为空，所以此处确保后缀名一直存在
        $ext = strtolower($file->getClientOriginalExtension()) ?: 'png';

        // 拼接文件名，加前缀是为了增加辨析度，前缀可以是相关数据模型的 ID
        // 值如：1_1493521050_7BVc9v9ujP.png
        $filename = $filePrefix . '_' . time() . '_' . str_random(10) . '.' . $ext;
